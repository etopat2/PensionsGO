...
class AdminDashboard {
    ...
    async openLoggedInUsersModal() {
        ...
        overlay.innerHTML = `
            <div class="admin-modal admin-modal-wide logged-in-users-modal" role="dialog" aria-modal="true" aria-labelledby="loggedInUsersModalTitle">
                ...
                <div class="settings-header" style="margin-top:1.25rem"><div><h3 class="section-title">Salary Scale Settings</h3><p class="section-subtitle">Maintain HRMIS salary scales used by staff-due intake and bulk uploads.</p></div><div class="settings-actions"><button class="action-btn secondary" id="refreshSalaryScalesBtn" type="button">Refresh</button><button class="action-btn" id="addSalaryScaleBtn" type="button">Add Salary Scale</button></div></div>
                <div class="settings-table-container"><table class="settings-table"><thead><tr><th>Scale</th><th>Description</th><th>Order</th><th>Status</th><th>Actions</th></tr></thead><tbody id="salaryScaleSettingsTableBody"><tr><td colspan="5"><div class="table-loading">Loading salary scales...</div></td></tr></tbody></table></div>
                <div class="settings-header" style="margin-top:1.25rem"><div><h3 class="section-title">Staff Reference Data</h3><p class="section-subtitle">Maintain selectable employment statuses, religions, and tribes used by staff intake.</p></div></div>
                <div id="staffReferenceSettingsGrid" class="settings-grid"></div>
                <div class="settings-header"><div><h4>Political Regions</h4><p class="section-subtitle">Regions used by political districts and staff home addresses.</p></div><button class="action-btn" id="addPoliticalRegionBtn" type="button">Add Region</button></div><div id="politicalRegionSettings"></div>
            </div>
        `;
    ...
    buildAdminSearchIndex() {
        ...
        return [
            ...
            entry('title-settings', 'Title & Salary Scale Settings', 'Manage staff titles and the HRMIS salary-scale catalogue.', 'titles rank salutation salary scales grade'),
    ...
    async fetchAppSettingsBundle(forceRefresh = false) {
        ...
        await this.loadSalaryScaleSettings();
        document.getElementById('refreshSalaryScalesBtn')?.addEventListener('click',()=>this.loadSalaryScaleSettings());
        document.getElementById('addSalaryScaleBtn')?.addEventListener('click',()=>this.editSalaryScaleSetting());
        document.getElementById('salaryScaleSettingsTableBody')?.addEventListener('click',(event)=>{const button=event.target.closest('button[data-scale-action]');if(!button)return;const scale=(this.salaryScaleSettings||[]).find((item)=>item.salary_scale_id===Number(button.dataset.id));if(!scale)return;if(button.dataset.scaleAction==='edit')this.editSalaryScaleSetting(scale);else if(button.dataset.scaleAction==='toggle')this.saveSalaryScaleSetting({...scale,is_active:!scale.is_active});else if(button.dataset.scaleAction==='delete'&&confirm(`Delete salary scale ${scale.scale_code}?`))this.saveSalaryScaleSetting(scale,'delete');});
        await this.loadStaffReferenceSettings();
        await this.loadPoliticalRegionSettings();
    }

    async loadSalaryScaleSettings(){const body=document.getElementById('salaryScaleSettingsTableBody');if(!body)return;try{const data=await fetch('../backend/api/get_salary_scales.php',{credentials:'include',cache:'no-store'}).then(r=>r.json());this.salaryScaleSettings=data.salary_scales||[];body.innerHTML=this.salaryScaleSettings.map(scale=>`<tr><td><strong>${this.escapeHtml(scale.scale_code)}</strong></td><td>${this.escapeHtml(scale.description||'')}</td><td>${scale.sort_order}</td><td>${scale.is_active?'Active':'Inactive'}</td><td><button class="action-btn secondary" data-scale-action="edit" data-id="${scale.salary_scale_id}">Edit</button> <button class="action-btn secondary" data-scale-action="toggle" data-id="${scale.salary_scale_id}">${scale.is_active?'Disable':'Enable'}</button> <button class="action-btn danger" data-scale-action="delete" data-id="${scale.salary_scale_id}">Delete</button></td></tr>`).join('')||'<tr><td colspan="5">No salary scales configured.</td></tr>';}catch(error){body.innerHTML='<tr><td colspan="5">Unable to load salary scales.</td></tr>';}}
    async editSalaryScaleSetting(scale=null){const code=prompt('Salary scale code:',scale?.scale_code||'');if(code===null||!code.trim())return;const description=prompt('Description:',scale?.description||'HRMIS Uganda Prisons salary scale');if(description===null)return;await this.saveSalaryScaleSetting({salary_scale_id:scale?.salary_scale_id||0,scale_code:code,description,sort_order:scale?.sort_order||((this.salaryScaleSettings?.length||0)+1),is_active:scale?.is_active??true});}
    async saveSalaryScaleSetting(scale,action='save'){try{const csrf=window.fetchCsrfToken?await window.fetchCsrfToken():'';const data=await fetch('../backend/api/manage_salary_scale.php',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf},body:JSON.stringify({...scale,action})}).then(r=>r.json());if(!data.success)throw new Error(data.message);this.showNotification(data.message,'success');await this.loadSalaryScaleSettings();}catch(error){this.showNotification(error.message||'Unable to save salary scale.','error');}}

    async loadStaffReferenceSettings(){const root=document.getElementById('staffReferenceSettingsGrid');if(!root)return;try{const response=await fetch('../backend/api/get_staff_reference_data.php',{credentials:'include',cache:'no-store'}).then(r=>r.json());const labels={employment_status:'Employment Statuses',religion:'Religions',tribe:'Tribes'};root.innerHTML=Object.entries(labels).map(([type,label])=>`<section class="settings-card"><div class="settings-header"><h4>${label}</h4><button class="action-btn" data-reference-add="${type}" type="button">Add</button></div><div class="settings-table-container"><table class="settings-table"><thead><tr><th>Value</th><th>Status</th><th>Actions</th></tr></thead><tbody>${(response.data?.[type]||[]).map(item=>`<tr><td>${this.escapeHtml(item.value)}</td><td>${item.is_active?'Active':'Inactive'}</td><td><button class="action-btn secondary" data-reference-edit="${type}" data-id="${item.id}">Edit</button> <button class="action-btn secondary" data-reference-toggle="${type}" data-id="${item.id}">${item.is_active?'Disable':'Enable'}</button> <button class="action-btn danger" data-reference-delete="${type}" data-id="${item.id}">Delete</button></td></tr>`).join('')}</tbody></table></div></section>`).join('');this.staffReferenceSettings=response.data||{};root.onclick=(event)=>{const button=event.target.closest('button');if(!button)return;const type=button.dataset.referenceAdd||button.dataset.referenceEdit||button.dataset.referenceToggle||button.dataset.referenceDelete;if(!type)return;const item=(this.staffReferenceSettings[type]||[]).find(row=>row.id===Number(button.dataset.id));if(button.dataset.referenceDelete){if(confirm(`Delete ${item?.value}?`))this.saveStaffReference(type,item,'delete');return;}if(button.dataset.referenceToggle){this.saveStaffReference(type,{...item,is_active:!item.is_active});return;}const value=prompt('Reference value:',item?.value||'');if(value!==null&&value.trim())this.saveStaffReference(type,{...item,value,sort_order:item?.sort_order||((this.staffReferenceSettings[type]?.length||0)+1),is_active:item?.is_active??true});};}catch(error){root.innerHTML='<p>Unable to load staff reference data.</p>';}}
    async saveStaffReference(type,item,action='save'){try{const csrf=window.fetchCsrfToken?await window.fetchCsrfToken():'';const data=await fetch('../backend/api/manage_staff_reference.php',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf},body:JSON.stringify({...item,type,action})}).then(r=>r.json());if(!data.success)throw new Error(data.message);this.showNotification(data.message,'success');await this.loadStaffReferenceSettings();}catch(error){this.showNotification(error.message||'Unable to save reference data.','error');}}

    async loadPoliticalRegionSettings(){const root=document.getElementById('politicalRegionSettings');if(!root)return;const response=await fetch('../backend/api/get_staff_reference_data.php?type=political_region',{credentials:'include',cache:'no-store'}).then(r=>r.json());const rows=response.data?.political_region||[];root.innerHTML=`<div class="settings-table-container"><table class="settings-table"><thead><tr><th>Region</th><th>Status</th><th>Actions</th></tr></thead><tbody>${rows.map(item=>`<tr><td>${this.escapeHtml(item.value)}</td><td>${item.is_active?'Active':'Inactive'}</td><td><button class="action-btn secondary" data-pr-edit="${item.id}">Edit</button> <button class="action-btn secondary" data-pr-toggle="${item.id}">${item.is_active?'Disable':'Enable'}</button> <button class="action-btn danger" data-pr-delete="${item.id}">Delete</button></td></tr>`).join('')}</tbody></table></div>`;this.politicalRegionSettings=rows;document.getElementById('addPoliticalRegionBtn').onclick=()=>this.editPoliticalRegion();root.onclick=event=>{const button=event.target.closest('button');if(!button)return;const id=Number(button.dataset.prEdit||button.dataset.prToggle||button.dataset.prDelete);const item=rows.find(row=>row.id===id);if(button.dataset.prDelete&&confirm(`Delete ${item.value}?`))this.savePoliticalRegion(item,'delete');else if(button.dataset.prToggle)this.savePoliticalRegion({...item,is_active:!item.is_active});else if(button.dataset.prEdit)this.editPoliticalRegion(item);};}
    editPoliticalRegion(item=null){const value=prompt('Political region:',item?.value||'');if(value!==null&&value.trim())this.savePoliticalRegion({...item,value,sort_order:item?.sort_order||((this.politicalRegionSettings?.length||0)+1),is_active:item?.is_active??true});}
    async savePoliticalRegion(item,action='save'){try{const csrf=window.fetchCsrfToken?await window.fetchCsrfToken():'';const data=await fetch('../backend/api/manage_staff_reference.php',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf},body:JSON.stringify({...item,type:'political_region',action})}).then(r=>r.json());if(!data.success)throw new Error(data.message);this.showNotification(data.message,'success');await this.loadPoliticalRegionSettings();}catch(error){this.showNotification(error.message||'Unable to save political region.','error');}}
                ...
                <form class="admin-modal-body" id="userManagementForm">
                    <div class="form-grid">
                        ...
                        <div class="form-field">
                            ...
                            <input type="password" name="${isEdit ? 'newPassword' : 'userPassword'}" ${isEdit ? '' : 'required'} value="${isEdit ? '' : 'Prisons123'}" placeholder="${isEdit ? 'At least 6 characters' : 'Prisons123'}">
                            ${!isEdit ? '<small class="field-help">Defaults to Prisons123. You can replace it before creating the account.</small>' : ''}
                        </div>
                        <div class="form-field form-span">
        ...
        <div class="settings-content live-chat-settings">
            ...
            <form id="liveChatSettingsForm" class="settings-grid">
                ...
                <section class="settings-card">
                    ...
                    <div class="settings-fields">
                        ${numberField('live_chat_message_poll_ms', 'Message Poll Interval (ms)', 750, 5000, 100, 'How often open conversations check for new messages.')}
                        ${numberField('live_chat_receipt_poll_ms', 'Receipt Poll Interval (ms)', 1000, 5000, 100, 'How often sent messages reconcile delivered/read state.')}
                        ${numberField('live_chat_call_poll_ms', 'Call Poll Interval (ms)', 750, 10000, 100, 'How often the app checks for incoming or changed calls.')}
                        ${numberField('live_chat_signal_poll_ms', 'Call Signal Poll Interval (ms)', 200, 5000, 50, 'How often active calls exchange WebRTC signaling updates.')}
                    </div>
                </section>
                ...
                <section class="settings-card">
                    ...
                    <div class="settings-fields">
                        ...
                        ${numberField('public_chat_poll_interval_ms', 'Public Poll Interval (ms)', 1000, 5000, 100, 'How often visitor and agent public chat windows check for updates.')}
                        ...
                        <label class="settings-field">
                            ...
                            <input name="public_chat_allowed_attachment_types" placeholder="pdf,doc,docx,xls,xlsx,csv,txt,jpg,jpeg,png,gif,webp,mp3,wav,ogg,m4a,webm,mp4,mov">
