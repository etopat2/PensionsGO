...
function initDashboardController() {
  ...
  function buildAnalyticsPrintDocument(columns, sourceRows, columnLabels) {
    ...
    return window.PensionsGoExports?.buildPrintDocument
      ...
      : `
        ...
        <body><h1>${escapeHtml(analyticsRecordsState.title)}</h1><table><thead><tr><th>S/N</th>${headers.map((header) => `<th>${escapeHtml(header)}</th>`).join("")}</tr></thead>
        <tbody>${rows.map((row, index) => `<tr><td>${index + 1}</td>${headers.map((header) => `<td>${escapeHtml(row[header] ?? "")}</td>`).join("")}</tr>`).join("")}</tbody></table><footer>Exported from UPSPensionsGo</footer></body></html>
      `;
  }
  ...
  async function renderWorkflowPerformance() {
    ...
    try {
      ...
      const data = await parseDashboardJsonResponse(response, "Unable to load workflow performance metrics.");
      if (!response.ok || !data.success) {
      ...
      if (staffDueSummaryResponse) {
        try {
          const staffDueData = await parseDashboardJsonResponse(staffDueSummaryResponse, "Unable to load staff due summary.");
  ...
  function summaryReportDocumentStyles() {
    return `
      ...
      * { box-sizing: border-box; }
      ...
      .summary-report-table th:first-child, .summary-report-table td:first-child {
        width: 48px;
        text-align: center;
      }
  ...
  function createSummaryReportHtml(model, options = {}) {
    ...
    return `<!DOCTYPE html>
      ...
      <body>
        ${contentMarkup}
        <footer class="summary-report-footer">Exported from UPSPensionsGo</footer>
      </body>
      </html>`;
  }
