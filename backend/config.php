...
function enforceParsedRowLimit(mysqli $conn, int $rowCount, string $label = 'Imported dataset', int $minimumAllowedRows = 0): void {
    $maxRows = max(getSecurityMaxImportRows($conn), $minimumAllowedRows);
    if ($rowCount > $maxRows) {
...
function ensureStaffDueExtendedColumns(mysqli $conn): void {
    ...
    $columns = [
        ...
        ,'positionName' => "VARCHAR(160) DEFAULT NULL"
        ,'firstName' => "VARCHAR(100) DEFAULT NULL"
        ,'middleName' => "VARCHAR(100) DEFAULT NULL"
        ,'lastName' => "VARCHAR(100) DEFAULT NULL"
        ,'next_of_kin_nin' => "VARCHAR(50) DEFAULT NULL"
        ,'salaryScale' => "VARCHAR(80) DEFAULT NULL"
        ,'employmentStatus' => "VARCHAR(80) DEFAULT NULL"
        ,'tribe' => "VARCHAR(100) DEFAULT NULL"
        ,'homeDistrict' => "VARCHAR(120) DEFAULT NULL"
        ,'homeRegion' => "VARCHAR(120) DEFAULT NULL"
        ,'religion' => "VARCHAR(100) DEFAULT NULL"
        ,'country' => "VARCHAR(100) DEFAULT NULL"
        ,'subCounty' => "VARCHAR(120) DEFAULT NULL"
        ,'parish' => "VARCHAR(120) DEFAULT NULL"
...
function pensionNumberFromEmployeeNumber(?string $employeeNumber): string {
    ...
    $value = preg_replace('#^PEN/#i', '', $value);
...
function ensureStaffVerificationTables(mysqli $conn): void {
    $conn->query("
        CREATE TABLE IF NOT EXISTS tb_staff_verification_documents (
            verification_document_id INT NOT NULL AUTO_INCREMENT,
            ...
            PRIMARY KEY (verification_document_id),
...
function ensurePensionBeneficiaryTables(mysqli $conn): void {
    $conn->query("
        CREATE TABLE IF NOT EXISTS tb_pension_beneficiaries (
            beneficiary_id INT NOT NULL AUTO_INCREMENT,
            deceased_staffdue_id INT DEFAULT NULL,
            deceased_registry_id INT DEFAULT NULL,
            deceased_ipps_no VARCHAR(80) NOT NULL,
            beneficiary_type VARCHAR(50) NOT NULL DEFAULT 'administrator',
            first_name VARCHAR(100) NOT NULL,
            middle_name VARCHAR(100) DEFAULT NULL,
            last_name VARCHAR(100) NOT NULL,
            beneficiary_nin VARCHAR(50) NOT NULL,
            beneficiary_ipps_no VARCHAR(80) DEFAULT NULL,
            beneficiary_supplier_no VARCHAR(80) DEFAULT NULL,
            telephone VARCHAR(50) DEFAULT NULL,
            email VARCHAR(120) DEFAULT NULL,
            relationship_to_deceased VARCHAR(80) DEFAULT NULL,
            administration_reference VARCHAR(120) DEFAULT NULL,
            earning_start_date DATE DEFAULT NULL,
            earning_end_date DATE DEFAULT NULL,
            ...
            KEY idx_beneficiary_deceased_ipps (deceased_ipps_no),
            KEY idx_beneficiary_nin (beneficiary_nin),
            KEY idx_beneficiary_supplier (beneficiary_supplier_no),
...
function ensureFileMovementTables(mysqli $conn): void {
    ...
    $movementColumns = [
        'file_type' => "VARCHAR(30) NOT NULL DEFAULT 'pension'",
        'source_registry' => "VARCHAR(50) DEFAULT NULL",
        'destination_registry' => "VARCHAR(50) DEFAULT NULL",
        'movement_direction' => "VARCHAR(20) NOT NULL DEFAULT 'out'",
        'parent_movement_id' => "INT(11) DEFAULT NULL"
    ];
    foreach ($movementColumns as $column => $definition) {
        $result = $conn->query("SHOW COLUMNS FROM tb_file_movements LIKE '{$column}'");
        if ($result && $result->num_rows === 0) $conn->query("ALTER TABLE tb_file_movements ADD COLUMN {$column} {$definition}");
    }
    ...
    $conn->query("
        CREATE TABLE IF NOT EXISTS tb_service_files (
            ...
            bunch_reference VARCHAR(120) DEFAULT NULL,
            availability_status VARCHAR(40) NOT NULL DEFAULT 'not_availed',
            current_holder VARCHAR(160) DEFAULT NULL,
            availed_at DATETIME DEFAULT NULL,
            pension_file_created_at DATETIME DEFAULT NULL,
            gratuity_paid_at DATETIME DEFAULT NULL,
            ...
            UNIQUE KEY uq_service_file_staff (staffdue_id),
            UNIQUE KEY uq_service_file_employee (employeeNo),
            KEY idx_service_registry_stage (registry_stage),
