...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $conn->query("INSERT IGNORE INTO tb_service_files (staffdue_id, employeeNo, pensionNo, registry_stage, availability_status) SELECT id, employeeNo, regNo, 'pending_processing', 'not_availed' FROM tb_staffdue WHERE COALESCE(is_deleted,0)=0 AND COALESCE(TRIM(employeeNo),'')<>''");
    $stage = trim((string)($_GET['stage'] ?? ''));
    $search = trim((string)($_GET['search'] ?? ''));
    $sql = "SELECT sf.*, sd.firstName, sd.middleName, sd.lastName, sd.rankName, sd.positionName, sd.rankPosition, sd.prisonUnit FROM tb_service_files sf JOIN tb_staffdue sd ON sd.id=sf.staffdue_id WHERE COALESCE(sd.is_deleted,0)=0";
    $params=[]; $types='';
    if ($stage !== '') { $sql .= ' AND sf.registry_stage=?'; $params[]=$stage; $types.='s'; }
    if ($search !== '') { $sql .= ' AND (sf.employeeNo LIKE ? OR sf.pensionNo LIKE ? OR sf.shelf_reference LIKE ? OR sd.firstName LIKE ? OR sd.lastName LIKE ?)'; $term="%{$search}%"; $params=array_merge($params,array_fill(0,5,$term)); $types.='sssss'; }
    $sql .= ' ORDER BY sf.updated_at DESC, sf.service_file_id DESC LIMIT 500';
...
$staffStmt=$conn->prepare('SELECT id, employeeNo, regNo FROM tb_staffdue WHERE id=? AND COALESCE(is_deleted,0)=0 LIMIT 1');
...
$sql="INSERT INTO tb_service_files (staffdue_id,employeeNo,pensionNo,registry_stage,shelf_reference,bunch_reference,availability_status,notes,updated_by,{$dateColumn},updated_at) VALUES (?,?,?,?,?,?,?,?,?,NOW(),NOW()) ON DUPLICATE KEY UPDATE pensionNo=VALUES(pensionNo),registry_stage=VALUES(registry_stage),shelf_reference=VALUES(shelf_reference),bunch_reference=VALUES(bunch_reference),availability_status=VALUES(availability_status),notes=VALUES(notes),updated_by=VALUES(updated_by),{$dateColumn}=NOW(),updated_at=NOW()";
$stmt=$conn->prepare($sql); $stmt->bind_param('issssssss',$staffId,$staff['employeeNo'],$staff['regNo'],$stage,$shelf,$bunch,$availability,$notes,$_SESSION['userId']); $ok=$stmt->execute();
