...
if (!$settings['enabled']) {
    publicChatJson(['success' => false, 'message' => 'Public support is currently disabled.'], 403);
}
...
publicChatRateLimit($conn, 'offline', publicChatSettingInt($conn, 'public_chat_rate_limit_start_per_10min', 5, 1, 50), 600);
...
if (!$consentAccepted) {
    publicChatJson(['success' => false, 'message' => 'Consent is required before submitting an offline support request.'], 400);
}
...
$closeReason = 'Offline message submitted for follow-up';
...
try {
    ...
    $stmt = $conn->prepare("
        ...
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'closed', 'normal', ?, ?, NOW(), NOW(), ?)
    ");
    if (!$stmt) {
        throw new RuntimeException('Unable to create offline request.');
    }
    $stmt->bind_param('ssssssssssissssss', $reference, $name, $phone, $email, $forceNo, $pensionerNo, $district, $location, $category, $subject, $consentAccepted, $sourcePage, $ip, $ua, $pensionerUserId, $contextJson, $closeReason);
    $stmt->execute();
    $sessionId = (int)$stmt->insert_id;
    $stmt->close();

    $msgStmt = $conn->prepare("INSERT INTO public_chat_messages (session_id, sender_type, sender_name, message_text) VALUES (?, 'visitor', ?, ?)");
    $msgStmt->bind_param('iss', $sessionId, $name, $message);
    $msgStmt->execute();
    $msgStmt->close();

    $ticketRef = 'PCT-' . date('YmdHis') . '-' . $sessionId;
    $ticketStmt = $conn->prepare("INSERT INTO public_chat_tickets (session_id, ticket_reference, subject, description, priority) VALUES (?, ?, ?, ?, 'normal')");
    $ticketStmt->bind_param('isss', $sessionId, $ticketRef, $subject, $message);
    $ticketStmt->execute();
    $ticketStmt->close();

    publicChatAudit($conn, $sessionId, 'Chat created', ['chat_reference' => $reference, 'offline' => true, 'inquiry_category' => $category]);
